...
1. **`plasm_context`** — **first call**. On semantic auto-seed hosts: `session_mode: "new"` + **`intent` only** (do **not** invent `{api, entity}` seeds). Returns **`logical_session_ref`** + language card (`e#`, `m#`, `r#` + wire names for fields/params). On clarify/hard_miss, rephrase intent with the provider brand. Use `seeds` on **`extend`**, or on `new` only when auto-seed is off.
2. **`plasm`** — dry-run a Plasm **program** using symbols from the language card. Returns **`run_ref`** (`pcN`).
...
- Copy symbols from the **left column** of language card into programs — never invent `e#` / `m#` / `r#` or untaught wire names or untaught catalog English names.
