...
- `plasm`: Pass **`logical_session_ref`** and **`program`**. Runs Plasm lines using the session’s exposure map when present. Paginated lists: follow **`page(l_<token>_pgN)`** (and `_meta.plasm.paging`) — the prefix must match your `logical_session_ref`.
- **`plasm_run`**: Live execute (blocking by default). With **`wait: false`**, returns **`wait(l_<token>_oN)`** immediately; poll with **`wait(l_<token>_oN)`**, cancel with **`cancel(l_<token>_oN)`**. When dry verdict is **review**, pass **`plan_commit_ref`** (`pcN`) from matching **`plasm`** dry-run or **`force: true`**. See [plasm-long-operations.md](plasm-long-operations.md) and grammar in [plasm-language-definition.md](plasm-language-definition.md#host-continuations-page-wait-cancel).

The first-wave teaching TSV preamble (via [`render_prompt_contract`](https://github.com/PlasmTools/plasm-core/blob/main/crates/plasm-core/src/prompt_render.rs)) teaches **`page(l_<token>_pgM)`**, **`wait(l_<token>_oM)`**, and **`cancel(l_<token>_oM)`** alongside entity/query grammar — same contract as MCP [`program_contract.txt`](https://github.com/PlasmTools/plasm-core/blob/main/crates/plasm-agent-core/src/mcp_prompt/program_contract.txt) and Phoenix tool-model `execute` notes ([tool-model-http.md](tool-model-http.md)).
