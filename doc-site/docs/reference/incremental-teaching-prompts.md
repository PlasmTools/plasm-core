...
**Intent-scoped exposure** (when `plasm_context` sets `context_intent`): the teaching surface contains exactly the selected business capabilities, Jev-selected input-source capabilities, and declared infrastructure prerequisites. [`selected_capability_surface`](https://github.com/PlasmTools/plasm-core/blob/main/crates/plasm-core/src/capability_exposure.rs) does not admit sibling reads, entity-wide mutators, or merely type-compatible producers. Live open/federate/expand and NAPI `expose_routing` share this one surface. Explicit entity-seeded execution remains entity-wide by construction.
