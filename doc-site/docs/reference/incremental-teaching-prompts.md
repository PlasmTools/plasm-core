...
MCP initialize teaches **`page(l_<token>_pgM)`**, **`wait(l_<token>_oM)`**, and **`cancel(l_<token>_oM)`** alongside entity/query grammar. Phoenix tool-model `execute` notes mirror the same continuation semantics ([tool-model-http.md](tool-model-http.md)).
