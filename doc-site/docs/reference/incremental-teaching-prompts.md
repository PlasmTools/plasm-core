...
- **Wire names** — Fields, capability params, and query filters appear as catalog wire tokens in the teaching TSV left column (scoped under `e#` / `m#`).
...
- `plasm_context`: **Call first** on each MCP connection. Pass **`session_mode: "new"`** once per workflow (or **`"extend"`** + **`logical_session_ref`** on later turns) and required **`intent`**. See [mcp-session-reuse.md](mcp-session-reuse.md).
  - **Semantic auto-seed hosts** (default **`plasm-server`** / hosts with `PLASM_DISCOVERY_SEMANTIC_AUTO_SEED=1`): **`new` and intent-only `extend` take `intent` only — `seeds` are rejected**. The host routes seeds; **`ready`** mints/continues `logical_session_ref` + teaching TSV. Clarify / hard_miss do not mint a session.
