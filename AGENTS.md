...
- **Appliance**: `cargo run -p plasm-server -- --no-tui --schema apis/<x>` boots headless, **auto-starts an embedded Postgres** (`pg-embed`), and serves HTTP+MCP on `127.0.0.1:3000` (`/v1/health`, `/v1/registry`, `/execute`). The single-file fixture `fixtures/schemas/capability_with_input.cgs.yaml` referenced in the README is stale (parse error); pass a split catalog directory instead.
