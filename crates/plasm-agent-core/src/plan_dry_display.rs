...
fn surface_compact_expr(surface: &ValidatedSurfaceNode, es: Option<&ExecuteSession>) -> String {
    ...
    compact_agent_surface_expr(&raw)
}
...
mod tests {
    ...
    fn compact_agent_surface_expr_summarizes_large_heredoc() {
        ...
        let expr = format!("write <<PLASM_BODY\n{body}\nPLASM_BODY");
        let compact = compact_agent_surface_expr(&expr);
        assert!(
            !compact.contains(&body),
            "agent dry text must not inline large heredoc bodies"
        );
        assert!(
            compact.contains("chars, full in run artifact"),
            "expected char summary, got: {compact}"
        );
    }
}
