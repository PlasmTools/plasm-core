...
async fn mcp_apply_capability_seeds_federates_multi_catalog_and_dry_runs_distinct_e_symbols() {
    ...
    for (sym, entry_id) in [("e1", "github"), ("e2", "linear")] {
        let plan = compile_plasm_surface_line_to_plan(pipeline, None, &es, sym, sym)
        ...
        let bundle = compile_plasm_expression(pipeline, None, &es, sym, sym)
...
async fn mcp_async_wait_poll_reaches_terminal_result() {
    ...
    let running = try_dispatch_operation_program(&es, None, Some(&trace), &wait_program)
        .await
    ...
    let terminal = try_dispatch_operation_program(&es, None, Some(&trace), &wait_program)
        .await
