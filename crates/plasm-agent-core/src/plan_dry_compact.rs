...
//!
//! Dry `content` is a **view** of the plan: heredocs are always summarized; other long
//! literals use a length threshold. The full program remains on the plan commit for `plasm_run`.

use std::fmt::Write as _;

const INLINE_MAX: usize = 256;
const HEAD: usize = 64;
const FULL_RETAINED: &str = "full in plan commit";

/// Shrink large literal/heredoc bodies in agent-facing dry-run step lines.
///
/// Heredocs are always summarized (document/markdown payloads travel as heredocs in Plasm).
/// Other long literals keep the length threshold.
pub(crate) fn compact_agent_surface_expr(expr: &str) -> String {
    let after_heredocs = compact_heredocs(expr);
    summarize_agent_surface_literal(&after_heredocs, INLINE_MAX, HEAD)
}
    ...
    match value {
        serde_json::Value::String(s) => {
            if s.chars().count() > INLINE_MAX {
                serde_json::json!(format!("… ({} chars, {FULL_RETAINED})", s.chars().count()))
            } else {
                serde_json::Value::String(s)
            }
        }
...
fn compact_heredocs(expr: &str) -> String {
    ...
    while !rest.is_empty() {
        ...
        if let Some(close_idx) = body_rest.lines().position(|line| {
            let t = line.trim();
            t == tag
                || t.strip_suffix(')').is_some_and(|s| s.trim_end() == tag)
                || t.strip_suffix(',').is_some_and(|s| s.trim_end() == tag)
                || t.strip_suffix('}').is_some_and(|s| s.trim_end() == tag)
        }) {
...
fn summarize_agent_surface_literal(s: &str, inline_max: usize, head: usize) -> String {
    ...
    format!("{head_s}… ({n} chars, {FULL_RETAINED})")
}
...
mod tests {
    ...
    fn compact_agent_surface_expr_summarizes_large_heredoc() {
        ...
        assert!(compact.contains("full in plan commit"));
        assert!(!compact.contains("full in run artifact"));
        assert!(!compact.contains(&body));
    }

    #[test]
    fn compact_agent_surface_expr_summarizes_short_total_with_heredoc() {
        let expr = "pr_create(body=<<BODY\nhello world markdown\nmore lines here\nBODY\n)";
        let compact = compact_agent_surface_expr(expr);
        assert!(compact.contains("full in plan commit"), "{compact}");
        assert!(!compact.contains("hello world markdown"), "{compact}");
    }
}
