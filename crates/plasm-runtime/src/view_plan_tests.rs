...
#[test]
fn fixture_runner_node_field_where_picks_matching_row() {
    let cgs = matrix_views_cgs();
    let mut results = indexmap::IndexMap::new();
    results.insert(
        "items".into(),
        ExecutionResult {
            entities: vec![
                stub_item_row("other", "Nope"),
                stub_item_row("item-1", "Alpha"),
            ],
            count: 2,
            has_more: false,
            pagination_resume: None,
    ...
    let ambient = ViewAmbientContext::default();
    let (proof, _) =
        run_view_dag_sync(&runner, "lang_key_pick", scope, &cgs, &ambient).expect("pick");
    assert_eq!(
...
#[test]
fn fixture_runner_node_field_where_zero_match_errors() {
    let cgs = matrix_views_cgs();
    let runner = FixtureViewNodeRunner {
        results: indexmap::IndexMap::from([(
            "items".into(),
            ExecutionResult {
                entities: vec![stub_item_row("other", "Nope")],
                count: 1,
                has_more: false,
                pagination_resume: None,
                paging_handle: None,
    ...
    assert!(err.to_string().contains("node_field_where"), "{err}");
}
