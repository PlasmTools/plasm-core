...
/// Compact executable syntax for MCP `tools/list` when initialize is not model-visible.
pub fn render_plasm_mcp_tool_syntax_contract() -> String {
    format!(
        "**{marker}** Write one raw expression such as `{example}`, or multiline bindings with final roots. Copy left cells from `plasm_context` TSV, substitute placeholders, then chain with `.r#`/`.wire`, `[p#]`, `.filter{{…}}`, `.sort(...)`, `.limit(...)`, etc. If a plan merely echoes an object/array/string literal, that is a literal no-op; rewrite as Plasm source.\n",
        marker = MCP_TOOL_SYNTAX_CONTRACT_MARKER,
        example = MCP_TOOL_SYNTAX_EXAMPLE,
    )
}

/// One-line JSON-schema description for the MCP `plasm` tool `program` parameter.
pub fn render_plasm_mcp_program_param_description() -> String {
    format!(
        "Plasm source string, not JSON data. Example: `{example}`; replace symbols from `plasm_context` TSV.",
        example = MCP_TOOL_PARAM_EXAMPLE,
    )
}

/// Render the full agent-facing Plasm language guide (eval, `plasm init`, non-MCP surfaces).
///
/// Catalogue-specific `plasm_context` results still provide the active symbol table and may return
/// a narrower frontmatter for the exposed entity slice. MCP `tools/list` uses
/// [`render_plasm_mcp_tool_syntax_contract`] instead; this guide is deliberately generated by the
/// same renderer as those per-session TSV frontmatters.
