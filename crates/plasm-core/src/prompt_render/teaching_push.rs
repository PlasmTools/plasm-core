...
/// Noun-card shape row (`e#[wires]` / `noun · …`) — may bypass parse when bare query form is absent.
pub(crate) fn push_noun_card_teaching_row(
    gloss_emit: &mut Option<GlossScratch<'_>>,
    teaching_rows: &mut Vec<EntityTeachingExprRow>,
    _collect_meta: bool,
    cgs: &CGS,
    ...
    let gloss = Some(super::teaching_legend::NOUN_CARD_LEGEND_MARK.to_string());
    if let Some(gs) = gloss_emit.as_mut() {
        gs.emit_before_teaching_example(expr, None, gloss.as_deref(), &[]);
    }
    let mut teaching_line = teaching_expr_line_from_layers(
        expr,
        gloss.as_deref(),
    ...
    let dedupe_key = TeachingRowDedupeKey::new(expr, gloss.as_ref(), None);

    // Always Projection: coverage attributes primary Get; bare `e#[…]` may not typecheck on Get-only.
    let expression = domain_line_validate_cached(
        line_valid_cache,
    ...
    .map(|(_parsed, work)| work)
    .unwrap_or_else(|| expr.to_string());
    let meta = TeachingLineMeta {
        expression,
        kind: DomainLineKind::Projection,
        source_capability: source_capability.map(|c| c.to_string()),
        cross_entity: None,
        relation_materialization: None,
    };
    teaching_line.arrow = super::ReturnArrow::classify(meta.kind, &teaching_line.result_type);
